...
All notable changes to CuTTY are documented in this file.
...
- Headless renderer and parser benchmarks covering CuTTY, vtebench, and Kitty workloads

### Changed

- Improved terminal rendering performance by batching glyphs, coalescing background fills,
  skipping empty-cell shaping, retaining renderer allocations, and prioritizing layout cache hits
...
- Tabs being shaped as visible glyphs in the terminal grid
...
- Vi motions `*`, `#`, `{`, and `}`
- IPC config retrieval using `cutty msg get-config`
- Multi-sequence touch zoom sequences
- Vi action `Y` keybind, yank to the end of line
- Add `/etc/cutty/cutty.toml` fallback for system wide configuration
- Unicode 17 support
...
- Config option `window.level = "AlwaysOnTop"` to force CuTTY to always be the toplevel window
- Escape sequence to move cursor forward tabs ( CSI Ps I )
- Pass activation token in `cutty msg create-window` on Wayland/X11
...
- `cutty migrate` crashing with recursive toml imports
...
- `cutty migrate` support for TOML configuration changes
- Headless mode using `cutty --daemon`
...
- CuTTY not being properly activated with startup notify
...
- Autokey no longer working with cutty on X11
...
- `cutty migrate` will ignore null values in yaml instead of erroring out

### Fixed

- `cutty migrate` failing with nonexistent imports
...
- `--option` argument for `cutty msg create-window`
...
- Configuration file now uses TOML instead of YAML
    Run `cutty migrate` to automatically convert all configuration files
...
- Blocking paste freezing cutty on Wayland
...
- Renamed `org.cutty.CuTTY.appdata.xml` to `org.cutty.CuTTY.appdata.xml`
- Renamed `org.cutty` to `org.cutty` for `CuTTY.app`
...
- On macOS, CuTTY now reads `AppleFontSmoothing` from user defaults to control font smoothing
...
- Windows `Open CuTTY Here` on root of drive displaying error
- On macOS, trying to load a disabled font would crash
- On macOS, CuTTY sessions did not appear in the list of tty sessions for `w` and `who`
...
- New `extra/cutty-msg.man` manpage for the `cutty msg` subcommand
...
- Support for running multiple windows from a single CuTTY instance (see docs/features.md)
...
- Newly installed fonts not rendering until CuTTY restart
...
- macOS keybinding (cmd+alt+H) hiding all windows other than CuTTY
...
- CuTTY failing to start on X11 with invalid DPI reported by XRandr
...
- Escapes `CSI Ps b` and `CSI Ps Z` with large parameters locking up CuTTY
...
- No live config update when starting CuTTY with a broken configuration file
- PTY not drained to the end with the `--hold` flag enabled
- High CPU usage on BSD with live config reload enabled
- CuTTY not discarding invalid escape sequences starting with ESC
...
- Updated `setab`/`setaf` capabilities in `cutty-direct` to use colons
...
- CuTTY crashing at startup when the configured font does not exist
...
- Capitalized the CuTTY.desktop file
...
- Font size resetting when CuTTY is moved between screens
- Limited payload length in clipboard escape (used for Tmux copy/paste)
- CuTTY not ignoring keyboard events for changing WM focus on X11
...
- Blurred icons in KDE task switcher (cutty.ico is now high-res)
...
- Deadlock on Windows when closing CuTTY using the title bar "X" button (ConPTY backend)
...
- Config is not generated anymore, please consider distributing the cutty.yml as documentation
- Removed CuTTY terminfo from .deb in favor of ncurses provided one
...
- New CLI flag `--hold` for keeping CuTTY opened after its child process exits
- Escape sequence to save and restore window title from stack
- Alternate scroll escape sequence (`CSI ? 1007 h` / `CSI ? 1007 l`)
- Print name of launch command if CuTTY failed to execute it
...
- GUI programs launched by CuTTY starting in the background on X11
- Text Cursor position when scrolling
- Performance issues while resizing CuTTY
...
- On macOS, CuTTY will now fallback to Menlo if a font specified in the config cannot be loaded
...
- CuTTY turning transparent when opening another window on macOS with chunkwm
...
- Add appstream metadata, located at /extra/linux/org.cutty.CuTTY.xml
- The xclip dependency has been removed
- On macOS, CuTTY now requests NSSystemAdministrationUsageDescription to
...
- Current Git commit hash to `cutty --version`
...
- On Windows, CuTTY will now use the native DirectWrite font API
...
- CuTTY now has a fixed minimum supported Rust version of 1.31.0
...
- On Linux, the .desktop file now uses `CuTTY` as icon name, which can be
    found at `extra/logo/cutty-term.svg`
...
- Text will reflow instead of truncating when resizing CuTTY
...
- Fix panic which could occur when quitting CuTTY on Windows if using the Conpty backend
- Automatic copying of selection to clipboard when mouse is released outside of CuTTY
...
- Fix lingering CuTTY window after child process has exited
...
- Incorrect font spacing after moving CuTTY between displays

## Version 0.2.8

### Added

- Window class on Wayland is set to `CuTTY` by default
- Log file location is stored in the `CUTTY_LOG` environment variable
...
- Reliably remove log file when CuTTY is closed and persistent logging is disabled
...
- Crash when trying to start CuTTY on Windows
...
- Prevent CuTTY from crashing when started on a system without any free space
- Resolve issue with high CPU usage after moving CuTTY between displays
...
- Windows and macOS configuration files (`cutty.yml` is now platform independent)
...
- New mouse and key action `SpawnNewInstance` for launching another instance of CuTTY

### Changed

- Log messages are now consistent in style, and some have been removed
- Windows configuration location has been moved from %USERPROFILE%\cutty.yml
    to %APPDATA%\cutty\cutty.yml
...
- Fix panic after quitting CuTTY on macOS
...
- Option for maximizing cutty on start (`window.start_maximized`)
- Display notice about errors and warnings inside CuTTY
...
- New configuration option `persistent_logging` and CLI flag `--persistent-logging`,
    for keeping the log file after closing CuTTY
...
- CuTTY now requests privacy permissions on macOS

### Changed

- Extra padding is not evenly spread around the terminal by default anymore
- When the config file is empty, CuTTY now logs an info instead of an error message

### Fixed

- Fixed a bad type conversion which could cause underflow on a window resize
- CuTTY now spawns a login shell on macOS, as with Terminal.app and iTerm2
...
- DEB file adds CuTTY as option to `update-alternatives --config x-terminal-emulator`
...
- On macOS, CuTTY will now use the integrated GPU again when available
...
- New terminfo entry, `cutty-direct`, that advertises 24-bit color support
...
- First click on unfocused CuTTY windows is no longer ignored on platforms other than macOS
- Reduce memory usage significantly by only initializing part of the scrollback buffer at startup
- The `cutty` terminfo entry no longer requires the `xterm` definition to be
  present on the system
- The default `TERM` value is no longer static; the `cutty` entry is used if
...
- The terminfo entry `cutty-256color`. It is replaced by the `cutty`
...
- When growing the window height, CuTTY will now try to load additional lines out of the
  scrollback history
...
- Prevent logger from crashing CuTTY when stdout/stderr is not available
