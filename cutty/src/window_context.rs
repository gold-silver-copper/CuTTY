...
use cutty_terminal::event::Event as TerminalEvent;
use cutty_terminal::event_loop::{EventLoop as PtyEventLoop, Msg, Notifier};
use cutty_terminal::grid::{Dimensions, Scroll};
use cutty_terminal::index::Direction;
use cutty_terminal::sync::FairMutex;
use cutty_terminal::term::test::TermSize;
use cutty_terminal::term::{Term, TermMode};
use cutty_terminal::tty;
...
/// Event context for one individual CuTTY window.
...
impl WindowContext {
    ...
    fn new(
        ...
        // Create context for the CuTTY window.
        Ok(WindowContext {
            preserve_title,
            terminal,
            display,
    ...
    pub fn draw(&mut self, scheduler: &mut Scheduler) {
        ...
        if !self.display.visual_bell.completed() {
            // We can get an OS redraw which bypasses cutty's frame throttling, thus
