...
use cutty_terminal::event::{Event as TerminalEvent, EventListener, Notify};
use cutty_terminal::event_loop::Notifier;
use cutty_terminal::grid::{BidirectionalIterator, Dimensions, Scroll};
use cutty_terminal::index::{Boundary, Column, Direction, Line, Point, Side};
use cutty_terminal::selection::{Selection, SelectionType};
use cutty_terminal::term::cell::Flags;
use cutty_terminal::term::search::{Match, RegexSearch};
use cutty_terminal::term::{self, ClipboardType, Term, TermMode};
use cutty_terminal::vte::ansi::NamedColor;
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn spawn_new_instance(&mut self) {
        ...
        let cutty = env_args.next().unwrap();

        let mut args: Vec<String> = Vec::new();

        // Reuse the arguments passed to CuTTY for the new instance.
