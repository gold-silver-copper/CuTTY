...
use cutty_config::SerdeReplace;
use log::{LevelFilter, error};
use serde::{Deserialize, Serialize};
use toml::Value;

use cutty_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main CuTTY executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed CuTTY within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    pub embed: Option<String>,

    /// Specify alternative configuration file [default:
    /// $XDG_CONFIG_HOME/cutty/cutty.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\cutty\cutty.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/cutty/cutty.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: CuTTY].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: CuTTY].
...
pub enum SocketMessage {
    /// Create a new window in the same CuTTY process.
    CreateWindow(WindowOptions),

    /// Update the CuTTY configuration.
    Config(IpcConfig),
...
pub struct IpcConfig {
    ...
    #[clap(short, long, allow_hyphen_values = true, env = "CUTTY_WINDOW_ID")]
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in
            &[(Shell::Bash, "cutty.bash"), (Shell::Fish, "cutty.fish"), (Shell::Zsh, "_cutty")]
        {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "cutty", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/cutty.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "cutty", &mut file);
        // let mut file = File::create("../extra/completions/cutty.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "cutty", &mut file);
        // let mut file = File::create("../extra/completions/_cutty").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "cutty", &mut file);
    }
}
