...
const CUTTY_SOCKET_ENV: &str = "CUTTY_SOCKET";
...
impl IpcListener {
    pub fn new(
        ...
        // Register socket path as environment variable for `cutty msg`.
        unsafe { env::set_var(CUTTY_SOCKET_ENV, path.as_os_str()) };
        if options.daemon {
            println!("CUTTY_SOCKET={}; export CUTTY_SOCKET", path.display());
        }
...
/// Send a message to the active CuTTY socket.
...
fn find_socket(socket_path: Option<PathBuf>) -> IoResult<UnixStream> {
    ...
    if let Ok(path) = env::var(CUTTY_SOCKET_ENV) {
    ...
    for entry in fs::read_dir(socket_dir())?.filter_map(|entry| entry.ok()) {
        ...
        // Skip files that aren't CuTTY sockets.
