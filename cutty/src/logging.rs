...
pub const LOG_TARGET_IPC_CONFIG: &str = "cutty_log_window_config";

/// Name for the environment variable containing the log file's path.
const CUTTY_LOG_ENV: &str = "CUTTY_LOG";

/// Logging target for config error messages.
pub const LOG_TARGET_CONFIG: &str = "cutty_config_derive";

/// Logging target for winit events.
pub const LOG_TARGET_WINIT: &str = "cutty_winit_event";

/// Name for the environment variable containing extra logging targets.
///
/// The targets are semicolon separated.
const CUTTY_EXTRA_LOG_TARGETS_ENV: &str = "CUTTY_EXTRA_LOG_TARGETS";
...
fn extra_log_targets() -> &'static [String] {
    ...
    EXTRA_LOG_TARGETS.get_or_init(|| {
        env::var(CUTTY_EXTRA_LOG_TARGETS_ENV)
            .map_or(Vec::new(), |targets| targets.split(';').map(ToString::to_string).collect())
    })
}

/// List of targets which will be logged by CuTTY.
...
impl OnDemandLogFile {
    fn new() -> Self {
        ...
        path.push(format!("CuTTY-{}.log", process::id()));

        // Set log path as an environment variable.
        unsafe { env::set_var(CUTTY_LOG_ENV, path.as_os_str()) };
