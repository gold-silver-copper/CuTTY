...
This document gives an overview over CuTTY's features beyond its terminal
emulation capabilities. To get a list with supported control sequences take a
look at [CuTTY's escape sequence support](./escape_support.md).

## Vi Mode

The vi mode allows moving around CuTTY's viewport and scrollback using the
...
Search allows you to find anything in CuTTY's scrollback buffer. You can
...
either feeds them to an external application or triggers one of CuTTY's
built-in actions.
...
CuTTY supports running multiple terminal emulators from the same CuTTY
instance. New windows can be created either by using the `CreateNewWindow`
keybinding action, or by executing the `cutty msg create-window` subcommand.
